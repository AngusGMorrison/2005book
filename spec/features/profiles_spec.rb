...
RSpec.describe "New profile", type: :feature do
  ...
  it "preserves the edited name on subsequent edit" do
    edit_profile
    fill_in "profile_user_name", with: ""
    fill_in "profile_user_name", with: "Passed Test User"
    submit_edits
  ...
  it "preserves the selected mod on subsequent edit" do
    Mod.create(name: "Test Mod 2")
    edit_profile
    page.select("Test Mod 2", from: "profile_user_mod_id")
    submit_edits
  ...
  it "preserves the edited studies on subsequent edit" do
    edit_profile
    fill_in "profile_studies", with: "Coding"
    submit_edits
    edit_profile
    expect(page).to have_current_path(edit_profile_path(@slug1)).and have_content("Coding")
  end
  ...
  it "preserves the edited sex on subsequent edit" do
    edit_profile
    fill_in "profile_sex", with: "Bot"
    submit_edits
    edit_profile
    expect(page).to have_current_path(edit_profile_path(@slug1)).and have_content("Bot")
  end
